...
volumeNameSingle <- function(Region, Spp, RS = NULL, ...){
  ...
  if(is.null(Region))
    cli_abort(c("x" = "{.tmp Region} cannot be NULL."))
  if(is.null(Spp))
    cli_abort(c("x" = "{.tmp Spp} cannot be NULL."))
  ## stop_if_NA = TRUE & Spp = NA, then cause error
  ## stop_if_NA = TRUE & Region = NA, then cause error
  if(stop_if_NA){
    ...
    if(is.na(Region)) cli_abort(c("{.tmp Region} should be character.", "x" = "{.tmp Region} is NA/NaN."))
    if(!is.character(Region)) cli_abort(c("{.tmp Region} should be character.", "x" = "class {.tmp Region} is {.cls {class(Region)}}."))
  }
  ## stop_if_NA = FALSE & Spp = NA, then return NA
  ## stop_if_NA = FALSE & Region = NA, then return NA
  if(!stop_if_NA){
    if(is.na(Spp)){
    ...
    if(!is.character(Spp)) cli_abort(c("{.tmp Spp} should be character.", "x" = "class {.tmp Spp} is {.cls {class(Spp)}}."))
    if(is.na(Region)){
      cli_alert_warning(c("{.tmp Region} should be character, return {.tmp2 {.var {name_invalid}}}."))
      return(name_invalid)
    }
    if(!is.character(Region)) cli_abort(c("{.tmp Region} should be character.", "x" = "class {.tmp Region} is {.cls {class(Region)}}."))
  }
  ...
  if(str_detect(name_region, pattern = "^\\p{ascii}+$")) # only ASCII chars, convert to Japanese
    name_region <- convertRegionToJapanese(name_region)
  ...
  if(str_detect(name_spp, pattern = "^\\p{ascii}+$")) # only ASCII chars, convert to Japanese
    name_spp <- convertSppToJapanese(name_spp)
  # prepare for list RS ---
...
#'   Please see the original article by Hosoda et al. (2010) and description in the above program.
#'   Region/Prefecture name in Japanese or English (e.g., "東京" or "Tokyo").
#' @param Spp  Character vector. Species name in Japanese, English common name, or scientific name.
#'   (e.g., "スギ", "Japanese cedar", or "Cryptomeria japonica").
...
volumeName <- function(Region, Spp, RS = NULL, ...){
  ...
  # for multiple values ---
    ...
    ## check class ---
    if(any(!is.na(Region) & !is.character(Region)))
      cli_abort(c("{.tmp Region} should be character.",
                  "x" = "There are {sum(!is.na(Region) & !is.character(Region))} {.tmp Region} that are not {.cls character}."))
...
#' Convert species name to Japanese
#'
#' This function converts species name in English to Japanese.
#' @param name_spp  Character. Species name in English.
#' @return  Character. Species name in Japanese.

convertSppToJapanese <- function(name_spp){
  # Define a mapping of English species names to Japanese names
  list_name_map <- list(
    "スギ" = c("sugi", "japanese cedar", "cedar", "cryptomeria japonica"),
    "ヒノキ" = c("hinoki", "japanese cypress", "chamaecyparis obtusa"),
    "サワラ" = c("sawara", "sawara cypress", "chamaecyparis pisifera"),
    "アカマツ" = c("akamatsu", "japanese red pine", "pinus densiflora",
                   "kuromatsu", "japanese black pine", "pinus thunbergii"),
    "カラマツ" = c("karamatsu", "japanese larch", "larix kaempferi"),
    "モミ" = c("momi", "japanese fir", "abies firma"),
    "ツガ" = c("tsuga", "western hemlock", "tsuga sieboldii"),
    "トドマツ" = c("todomatsu", "sakhalin fir", "abies sachalinensis", "akatodomatsu", "aotodomatsu"),
    "エゾマツ" = c("ezomatsu", "Yezo spruce", "Picea jezoensis", "akaezomatsu", "akaezo", "kuroezomatsu", "kuroezo"),
    "ヒバ" = c("hiba", "asunaro", "thujopsis dolabrata", "nihokiasunaro"),
    "N" = c("N"),
    "ブナ" = c("buna", "japanese beech", "fagus crenata"),
    "イチョウ" = c("ichou", "ginkgo", "ginkgo biloba"),
    "マツ" = c("matsu", "pine", "pinus"),
    "シラビソ" = c("shirabiso", "maries fir", "abies mariesii"),
    "トウヒ" = c("touhi", "spruce", "picea"),
    "トガサワラ" = c("togasawara", "japanese douglas fir", "pseudotsuga japonica"),
    "コウヤマキ" = c("kouyamaki", "japanese umbrella pine", "sciadopitys verticillata"),
    "ビャクシン" = c("byakushin", "ibuki", "chinese juniper", "juniperus chinensis"),
    "ネズミサシ" = c("nezumisashi", "nezu", "temple juniper", "juniperus rigida"),
    "イヌマキ" = c("inumaki", "yew plum pine", "podocarpus macrophyllus"),
    "イヌガヤ" = c("inugaya", "japanese plum yew", "cephalotaxus drupacea"),
    "イチイ" = c("ichii", "japanese yew", "taxus cuspidata"),
    "天然スギ" = c("tennensugi", "tensugi", "natural cedar", "ten-sugi", "tennen-sugi"),
    "広葉樹" = c("broadleaf", "deciduous", "hardwood", "broad-leaved", "broadleaf tree", "broadleaf species",
                "broadleaf trees", "broadleaved", "broad-leaved tree", "broad-leaved species",
                "broadleaved tree", "broadleaved species", "deciduous tree", "deciduous species",
                "hardwood tree", "hardwood species")
  )


  # Convert the input species name to lowercase and check against the list
  name_spp_lower <- tolower(name_spp)
  # find the matching species name in the list
  wch_rep <- which(lapply(list_name_map, function(x) name_spp_lower %in% x) |> unlist())

  if(length(wch_rep) > 0) {
    name_spp <- names(list_name_map)[wch_rep]
  } else {
    name_spp <- name_spp # "広葉樹" # default name
  }

  return(name_spp)
}

#' Convert region name to Japanese
#' This function converts region name in English to Japanese.
#' @param name_region  Character. Region name in English.
#' @return  Character. Region name in Japanese.

convertRegionToJapanese <- function(name_region){

  # Convert region name to Japanese
  list_name_map <- list(
    "札幌" = c("hokkaido", "hokkaidou"),
    "旭川" = c("asahikawa", "asahikawa-shi"),
    "北見" = c("kitami", "kitami-shi"),
    "帯広" = c("obihiro", "obihiro-shi"),
    "札幌" = c("sapporo", "sapporo-shi"),
    "函館" = c("hakodate", "hakodate-shi"),
    "宗谷支庁" = c("souya", "souya-shichou"),
    "留萌支庁" = c("rumoi", "rumoi-shichou"),
    "上川支庁" = c("kamikawa", "kamikawa-shichou", "kamikawa-hokubu", "kamikawa-nanbu"),
    "雨竜地方" = c("uryuu", "uryuu-chihou"),
    "網走支庁" = c("abashiri", "abashiri-shichou"),
    "網走西部" = c("abashiri-seibu", "abashiri-west"),
    "網走東部" = c("abashiri-toubu", "abashiri-east"),
    "根室支庁" = c("nemuro", "nemuro-shichou"),
    "釧路支庁" = c("kushiro", "kushiro-shichou"),
    "十勝支庁" = c("tokachi", "tokachi-shichou"),
    "日高支庁" = c("hidaka", "hidaka-shichou"),
    "空知支庁" = c("sorachi", "sorachi-shichou"),
    "石狩支庁" = c("ishikari", "ishikari-shichou"),
    "胆振支庁" = c("iburi", "iburi-shichou"),
    "後志支庁" = c("shiribeshi", "shiribeshi-shichou"),
    "桧山支庁" = c("hiyama", "hiyama-shichou"),
    "渡島支庁" = c("oshima", "oshima-shichou"),
    "渡島檜山" = c("oshima-hiyama"),
    "後志胆振" = c("shiribeshi-iburi"),
    "胆振東部" = c("iburi-toubu", "iburi-higashi"),
    "十勝" = c("tokachi"),
    "釧路根室" = c("kushiro-nemuro"),
    "石狩空知" = c("ishikari-sorachi"),
    "幌別地方" = c("horobetsu", "horobetsu-chihou"),
    "白老地方" = c("shiraoi", "shiraoi-chihou"),
    "勇払地方" = c("yufutsu", "yufutsu-chihou"),
    "積丹地方" = c("shakotan", "shakotan-chihou"),
    "古平地方" = c("furubira", "furubira-chihou"),
    "余市地方" = c("yoichi", "yoichi-chihou"),
    "青森" = c("aomori", "aomori-ken"),
    "秋田" = c("akita", "akita-ken"),
    "岩手" = c("iwate", "iwate-ken"),
    "宮城" = c("miyagi", "miyagi-ken"),
    "山形" = c("yamagata", "yamagata-ken"),
    "福島" = c("fukushima", "fukushima-ken"),
    "茨城" = c("ibaraki", "ibaraki-ken"),
    "栃木" = c("tochigi", "tochigi-ken"),
    "群馬" = c("gunma", "gunma-ken"),
    "埼玉" = c("saitama", "saitama-ken"),
    "千葉" = c("chiba", "chiba-ken"),
    "東京" = c("tokyo", "tokyo-to"),
    "神奈川" = c("kanagawa", "kanagawa-ken"),
    "新潟" = c("niigata", "niigata-ken"),
    "富山" = c("toyama", "toyama-ken"),
    "石川" = c("ishikawa", "ishikawa-ken"),
    "福井" = c("fukui", "fukui-ken"),
    "山梨" = c("yamanashi", "yamanashi-ken"),
    "静岡" = c("shizuoka", "shizuoka-ken"),
    "長野" = c("nagano", "nagano-ken"),
    "岐阜" = c("gifu", "gifu-ken"),
    "愛知" = c("aichi", "aichi-ken"),
    "三重" = c("mie", "mie-ken"),
    "滋賀" = c("shiga", "shiga-ken"),
    "京都" = c("kyoto", "kyoto-fu"),
    "大阪" = c("osaka", "osaka-fu"),
    "兵庫" = c("hyogo", "hyogo-ken"),
    "奈良" = c("nara", "nara-ken"),
    "和歌山" = c("wakayama", "wakayama-ken"),
    "鳥取" = c("tottori", "tottori-ken"),
    "島根" = c("shimane", "shimane-ken"),
    "岡山" = c("okayama", "okayama-ken"),
    "広島" = c("hiroshima", "hiroshima-ken"),
    "山口" = c("yamaguchi", "yamaguchi-ken"),
    "徳島" = c("tokushima", "tokushima-ken"),
    "愛媛" = c("ehime", "ehime-ken"),
    "香川" = c("kagawa", "kagawa-ken"),
    "高知" = c("kochi", "kochi-ken"),
    "福岡" = c("fukuoka", "fukuoka-ken"),
    "佐賀" = c("saga", "saga-ken"),
    "長崎" = c("nagasaki", "nagasaki-ken"),
    "熊本" = c("kumamoto", "kumamoto-ken"),
    "大分" = c("oita", "oita-ken"),
    "宮崎" = c("miyazaki", "miyazaki-ken"),
    "鹿児島" = c("kagoshima", "kagoshima-ken"),
    "沖縄" = c("okinawa", "okinawa-ken"),
    "会津地方" = c("aizu", "aizu-chihou"),
    "会津新潟" = c("aizu-niigata", "aizu-shinetsu"),
    "関東中部" = c("kantou-chubu", "kantou-chubu-region", "kantou-chubu-area"),
    "前橋" = c("maebashi", "maebashi-shi"),
    "山陰" = c("sanin", "sanin-chihou"),
    "中丹地方" = c("chutan", "chutan-chihou"),
    "丹後地方" = c("tango", "tango-chihou"),
    "但馬地方" = c("tajima", "tajima-chihou"),
    "飫肥" = c("obi"),
    "飫肥地方" = c("obi-chihou", "obikata"),
    "下屋久" = c("shimo-yaku", "shimo-yakushima"),
    "下屋久地方" = c("shimo-yaku-chihou", "shimo-yakushima-chihou"),
    "上屋久" = c("kami-yaku", "kami-yakushima"),
    "上屋久地方" = c("kami-yaku-chihou", "kami-yakushima-chihou"),
    "霧島" = c("kirishima", "kirishima-shi"),
    "霧島地方" = c("kirishima-chihou", "kirishima-area")
  )

  # Convert the input region name to lowercase and check against the list
  name_region_lower <- tolower(name_region)
  # find the matching region name in the list
  wch_rep <- which(lapply(list_name_map, function(x) name_region_lower %in% x) |> unlist())

  if(length(wch_rep) > 0) {
    name_region <- names(list_name_map)[wch_rep]
  } else {
    name_region <- name_region # default name for unknown regions
  }
