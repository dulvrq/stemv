...
1. 胸高形数法における四捨五入計算の際、一部で浮動小数点の誤差が発生する（北海道針葉樹）。
...
- このうち、0.01m<sup>3</sup>以上の誤差は全て上記1. ~ 3. に起因し、0.0001m<sup>3</sup>以上の誤差は全て上記1. ~ 4.に起因します。

```{r, echo=FALSE}
kableExtra::kable(list_data_for_readme[[7]] |> dplyr::filter(`差≥0.0001の数` > 0),
                  caption = "差が存在する材積式における誤差")
```
...
- 小数点第2位を四捨五入して一致しない組み合わせは全体の`r round(100*list_data_for_readme[[5]]/list_data_for_readme[[4]], 1)`%です。
...
kableExtra::kable(list_data_for_readme[[8]] |> dplyr::arrange(dplyr::desc(`一致しない数`)) |> head(10),
              caption = "一致しない数が多い10種類の材積式における誤差")
```
